...
def existing_output_paths(out_dir, ph_mode):
    if not os.path.isdir(out_dir):
        return set()
    return {
        os.path.join(out_dir, name)
        for name in os.listdir(out_dir)
        if name.startswith(f"{ph_mode}__") and name.endswith(".csv")
    }
...
def measure_one_run_task(args):
    run_dir, out_dir, ph_mode = args
    out_path = output_path_for_run(run_dir, out_dir, ph_mode)
    ...
    try:
        ...
        df_run = measure_run(run_dir, too_big, ph_mode)
...
def main(
    ...
    print(f"[INFO] ph_mode={_ph_mode}")
    print(f"[INFO] workers={_workers}")

    tasks = [(run_dir, _out_dir, _ph_mode) for run_dir in run_dirs]

    results = []

    if _workers <= 1:
        for i, task in enumerate(tasks):
            print(f"[RUN {i + 1}/{len(tasks)}] {task[0]}", flush=True)
            results.append(measure_one_run_task(task))
    else:
        print(f"[DEBUG] run_dirs={len(run_dirs)}", flush=True)
        print(f"[DEBUG] existing={len(existing)}", flush=True)
        print(f"[DEBUG] tasks={len(tasks)}", flush=True)
        with ProcessPoolExecutor(max_workers=_workers) as ex:
            print(f"[PARENT] submitting {len(tasks)} tasks", flush=True)
            ...
            print("[PARENT] all tasks submitted", flush=True)
    ...
    status_path = os.path.join(_out_dir, f"{_ph_mode}_measurement_status.csv")
    results_df.to_csv(status_path, index=False)
    print(f"[DONE] wrote status: {status_path}")

    failed = results_df[results_df["status"] == "failed"]
    if len(failed) > 0:
        failed_path = os.path.join(_out_dir, f"{_ph_mode}_measurement_failed.csv")
        failed.to_csv(failed_path, index=False)
        print(f"[WARN] {len(failed)} runs failed; wrote {failed_path}")

    # Build combined all-metrics file from all successful/skipped per-run CSVs.
    csv_paths = [
        p for p in results_df["out_path"].dropna().tolist()
        if os.path.exists(p)
    ]

    frames = []
    for path in tqdm(csv_paths, desc="combining per-run CSVs"):
        try:
            df = pd.read_csv(path)
            if len(df) > 0:
                frames.append(df)
    ...
    if frames:
        ...
        all_path = os.path.join(_out_dir, f"{_ph_mode}_all_metrics.csv")
